...
export const UsersTable: React.FC = () => {
  const { filterUsers, isLoading, handleFetchUsers, handleDisabledUser, handleActiveUser, page } =
    useUserStore();
            ...
            <>
              <thead>
                <tr className="sticky top-0 bg-bgColor border-b border-primaryColor">
                  ...
                  <th className="min-w-[220px] h-12">Fecha de deshabilitación</th>
              ...
              <tbody>
                {filterUsers.slice((page - 1) * 10, page * 10).map((item: IUser, i: number) => (
                  <tr key={i} className="border-b border-admin-borderColor">
                    ...
                    <InfoRows item={item} label={item.disabledAt ? item.disabledAt : "-"} />
                    <td className="h-12">
                    ...
                    <InfoRows item={item} label={item.role && item.role.name} />
                    <td className="flex h-12 items-center gap-2 text-base text-letterColorLight">
                      ...
                      >
                        <FontAwesomeIcon icon={item.disabledAt ? faUser : faUserSlash} />
                      </button>

                      <button
                        className="bg-admin-activeColor w-8 h-8 rounded-[3px] disabled:bg-disabledButton"
                        disabled={
                          (item.installer?.status !== "APROBADO" && item.role.name !== "Usuario") ||
                          !!item.disabledAt
                        }
                      >
