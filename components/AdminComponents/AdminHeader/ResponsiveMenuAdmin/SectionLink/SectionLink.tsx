import { headerLinksAdmin } from "@/data/HeaderLinks/header-links";
...
export const SectionLink: React.FC = () => {
  ...
  return (
    <div className="flex flex-col">
      {headerLinksAdmin.map((link: IHeaderLink, i: number) => (
        <Link
          key={i}
        ...
        >
          {link.icon && (
            <FontAwesomeIcon
              icon={link.icon}
              className="w-[30px] lg:text-5xl"
            />
          )}

          <span className={clsx(menuAdmin ? "lg:block" : "lg:hidden")}>
            {link.label}
          </span>
        </Link>
      ))}
